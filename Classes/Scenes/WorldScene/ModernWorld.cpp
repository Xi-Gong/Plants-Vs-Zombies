...
ModernWorld::ModernWorld() :
    ...
    _backgroundSize(_director->getWinSize()), _parallax(nullptr), _scrollView(nullptr), _level(0) {
    srand(time(nullptr));
    _worldPosition = _userDataHandler->openUserData<double>("WORLD_1_POSITION");
}

ModernWorld::~ModernWorld() { _isPopEnter = false; }

bool ModernWorld::init() {
    if (!Scene::init()) {
        return false;
    }
...
void ModernWorld::onEnter() {
    ...
    if (_isPopEnter) {
        ...
        this->addChild(layer);
        layer->runAction(
            Sequence::create(DelayTime::create(0.1f), CallFunc::create([=]() {
                                 Director::getInstance()->replaceScene(
                                     TransitionFade::create(0.5f, ModernWorld::create()));
                             }),
                             nullptr));
    }

    /* 播放音乐 */
    MusicPlayer::changeBackgroundMusic("mainmusic2", true);
}

void ModernWorld::setPopEnter(const bool isPopEnter) { _isPopEnter = isPopEnter; }

void ModernWorld::readWorldLevel() {
    auto& resourcePath = _runtime->gameData->getResourcePath();
    auto& worldData = _runtime->userData->getUserWorldData();

    /* 读取该世界关卡数据（每次开始游戏都读取一次） */
    if (!_runtime->userData->getUserWorldData().isReadWoldInformation) {
        auto path = resourcePath.getTextPath().find("GAMEWORLD_1DATAS")->second;
        _levelDataHandler->openLevelData(path);
        worldData.isReadWoldInformation = true;
    }
    ...
    worldData.highestLevel = _userDataHandler->openUserData<int>(worldFile);

    if (worldData.highestLevel == 0) {
        worldData.highestLevel = 1;
    }
}

void ModernWorld::setLevelVisible(Node* node) {
    auto& worldData = _runtime->userData->getUserWorldData();
    ...
    if (worldData.highestLevel == _level) {
        node->setOpacity(100);
        node->setCascadeOpacityEnabled(true);
    }
    if (worldData.highestLevel == _level - 1) {
        node->setOpacity(30);
        node->setCascadeOpacityEnabled(true);
    }
}

void ModernWorld::createButtonListener(Button* button, const int ID) {
    button->addTouchEventListener([=](Ref* sender, ui::Widget::TouchEventType type) {
        switch (type) {
            case ui::Widget::TouchEventType::BEGAN: MusicPlayer::playMusic("tap"); break;
            case ui::Widget::TouchEventType::ENDED: playLevelGameAndCaveThings(ID); break;
            default: break;
        }
    });
}

Button* ModernWorld::createButton(Node* node, const std::string& name, const Vec2& position) {
    ...
    text->setFontSize(30);
    text->setString(std::to_string(++_level));
    ...
    wgfd->runAction(RepeatForever::create(
        Sequence::create(FadeOut::create(0.2f), FadeIn::create(0.2f), nullptr)));
    sprite4->runAction(RepeatForever::create(
        Sequence::create(TintTo::create(0.1f, Color3B(100, 100, 100)),
                         TintTo::create(0.1f, Color3B(255, 255, 255)), nullptr)));
    ...
    if (_global->userInformation->getUserSelectWorldData().at(0)->levels == _level)
    {
        ...
        wgfd->runAction(RepeatForever::create(Sequence::create(FadeOut::create(0.2f),
    FadeIn::create(0.2f), nullptr)));
        sprite4->runAction(RepeatForever::create(Sequence::create(TintTo::create(0.1f, Color3B(100,
    100, 100)), TintTo::create(0.1f, Color3B(255, 255, 255)), nullptr)));
    }
    if (_global->userInformation->getUserSelectWorldData().at(0)->levels > _level)
    {
        auto wgfd = Sprite::createWithSpriteFrameName("wgfd.png");
...
void ModernWorld::createScrollView() {
    ...
    _scrollView->setInnerContainerSize(Size(3310, size.height));  // 5个关卡的宽度
    ...
    back->addTouchEventListener([=](Ref* sender, ui::Widget::TouchEventType type) {
        switch (type) {
            ...
            case ui::Widget::TouchEventType::ENDED: {
                _userDataHandler->caveUserData("WORLD_1_POSITION",
                                               _scrollView->getScrolledPercentHorizontal());

                auto& userWorldData = _runtime->userData->getUserWorldData();
                userWorldData.mainToWorld = false;
                Director::getInstance()->replaceScene(
                    TransitionFade::create(0.5f, MainMenu::create()));
                break;
            }
...
void ModernWorld::showLevels() {
    /* 创建关卡 */
    auto World1_38 =
        createSprite("World1_38", Vec2(10, _backgroundSize.height / 2.0f), 1.7f, 2, 0.5f, false);
...
void ModernWorld::addScrollView(const int id) {
    /* 创建背景物品 */
    const string name[5] = {
        {"World1_32"}, {"World1_31"}, {"World1_36"}, {"World1_30"}, {"World1_27"}};
    const string name1[2] = {{"World1_22"}, {"World1_40"}};

    for (int i = 0; i < 10; i++) {
        const auto world_32 =
            createSprite(name[i % 5], Vec2(500 + 1000 * i, i % 2 == 0 ? 850 : 350),
                         rand() % 4 / 10.0f + 1.5f, 1, 0.2f, true);
        world_32->runAction(RepeatForever::create(RotateBy::create(10, 30)));
    }

    for (int i = 0; i < 8; i++) {
        const auto world_32 = createSprite(name1[i % 2], Vec2(500 + 2000 * i, 180),
                                           rand() % 3 / 10.0f + 1.5f, 3, 0.7f, true);
    ...
    Vec2 BeginPoint[53] =
    {
        Vec2(575, 450) ,Vec2(1050, 580) ,Vec2(1370, 730) ,Vec2(1650, 510) ,Vec2(2050, 440)
    ,Vec2(2300, 370) ,Vec2(2650, 520) ,Vec2(2900,740), Vec2(3230, 560) ,Vec2(3550, 520) ,Vec2(3920,
    580) ,Vec2(4250, 750) ,Vec2(4620, 660) ,Vec2(4960, 590) ,Vec2(5250, 460) ,Vec2(5620,620),
        Vec2(5950, 665) ,Vec2(6270, 770) ,Vec2(6600, 590) ,Vec2(6900, 490) ,Vec2(7200, 420)
    ,Vec2(7500, 520) ,Vec2(7810, 680) ,Vec2(8100, 790), Vec2(8400, 725) ,Vec2(8700, 600) ,Vec2(9000,
    510) ,Vec2(9300, 450) ,Vec2(9580, 540) ,Vec2(9950, 670) ,Vec2(10300, 750) ,Vec2(10590, 690),
        Vec2(10900, 620) ,Vec2(11200, 510) ,Vec2(11500, 400) ,Vec2(11900, 500) ,Vec2(12100, 410)
    ,Vec2(12300, 500) ,Vec2(12700, 430) ,Vec2(13000, 520), Vec2(13300, 620) ,Vec2(13600, 730)
    ,Vec2(13900, 660) ,Vec2(14200, 570) ,Vec2(14500, 470) ,Vec2(14800, 370) ,Vec2(15130, 540)
    ,Vec2(15500, 680), Vec2(15800, 730) ,Vec2(16200, 680) ,Vec2(16520, 480) ,Vec2(16780,
    390),Vec2(16900,480)
    };
    Vec2 EndPoint[53] =
    {
        Vec2(1000, 560) ,Vec2(1300, 700) ,Vec2(1600, 520) ,Vec2(2000, 440) ,Vec2(2250, 380)
    ,Vec2(2600, 510) ,Vec2(2900, 720) ,Vec2(3190,560), Vec2(3530, 520) ,Vec2(3900, 580) ,Vec2(4250,
    760) ,Vec2(4600, 650) ,Vec2(4960, 590) ,Vec2(5250, 460) ,Vec2(5600, 620) ,Vec2(5950,670),
        Vec2(6270, 750) ,Vec2(6600, 590) ,Vec2(6900, 490) ,Vec2(7200, 440) ,Vec2(7500, 520)
    ,Vec2(7810, 680) ,Vec2(8070, 795) ,Vec2(8400, 720), Vec2(8700, 600) ,Vec2(9000, 510) ,Vec2(9300,
    450) ,Vec2(9580, 540) ,Vec2(9950, 670) ,Vec2(10300, 750) ,Vec2(10590, 690) ,Vec2(10900, 620),
        Vec2(11200, 510) ,Vec2(11500, 400) ,Vec2(11900, 500) ,Vec2(12100, 410) ,Vec2(12300, 500)
    ,Vec2(12700, 430) ,Vec2(13000, 520) ,Vec2(13300, 620), Vec2(13600, 730) ,Vec2(13900, 660)
    ,Vec2(14200, 570) ,Vec2(14500, 470) ,Vec2(14800, 370) ,Vec2(15130, 540) ,Vec2(15500, 680)
    ,Vec2(15800, 730), Vec2(16200, 680) ,Vec2(16520, 480) ,Vec2(16700, 380) ,Vec2(16900,
    450),Vec2(16950,480)
    };
    for (int i = 0; i < _global->userInformation->getUserSelectWorldData().at(id)->levels; ++i)
    {
        ...
        draw->runAction(RepeatForever::create(Sequence::create(MoveBy::create(0.05f, Vec2(0.1f,
    0.1f)), MoveBy::create(0.05f, Vec2(-0.1f, -0.1f)), nullptr))); auto LineAction =
    Sprite::createWithSpriteFrameName("LineAction.png"); LineAction->setPosition(EndPoint[i]);
        draw->addChild(LineAction);
        LineAction->runAction(RepeatForever::create(Sequence::create(MoveTo::create(1.0f,
    EndPoint[i]), CallFunc::create([=]()
                {
                    LineAction->setPosition(BeginPoint[i]);
                }), nullptr)));
    }
    draw->runAction(RepeatForever::create(Sequence::create(FadeIn::create(0.7f),
    FadeOut::create(0.7f), nullptr))); _parallax->addChild(draw, 3, Vec2(0.5f, 0), Vec2(0, 0));*/

    showLevels();
}

Sprite* ModernWorld::createSprite(const std::string& name, const Vec2& position, const float scale,
                                  const int zorder, const float speed, bool IsFlipped) {
    auto sprite = Sprite::createWithSpriteFrameName(name + ".png");
    sprite->setAnchorPoint(Vec2(0, 0.5f));
    sprite->setScale(scale);
    sprite->setPosition(position);
    if (IsFlipped) {
        sprite->setFlippedX(rand() % 2);
    }
...
Sprite* ModernWorld::createSprite(Node* node, const std::string& name, const Vec2& position,
                                  const float scale, const int zorder, bool IsFlipped) {
    auto sprite = Sprite::createWithSpriteFrameName(name + ".png");
    sprite->setAnchorPoint(Vec2(0, 0.5f));
    sprite->setScale(scale);
    sprite->setPosition(position);
...
void ModernWorld::createMouseListener() {
...
void ModernWorld::playProhibitMusic(Button* button) {
    button->addTouchEventListener([=](Ref* sender, ui::Widget::TouchEventType type) {
        switch (type) {
            case ui::Widget::TouchEventType::BEGAN: {
            ...
            case ui::Widget::TouchEventType::ENDED: {
                _runtime->prohibitId =
                    cocos2d::experimental::AudioEngine::play2d("resources/Music/prohibit.ogg");
...
void ModernWorld::playLevelGameAndCaveThings(const int id) {
    ...
    // 读取关卡信息
    _levelDataHandler->createLevelData(id, levelName);
    _levelDataHandler->setLevelNumber(id);

    auto& userWorldData = _runtime->userData->getUserWorldData();
    userWorldData.currentLevel = id;

    _userDataHandler->createNewLevelDataDocument();
    /*if
    (UserData::getInstance()->isHaveLevelData(_global->userInformation->getCurrentCaveFileLevelWorldName()))
    {
        ...
        Director::getInstance()->pushScene(TransitionFade::create(1.0f,
    SelectPlantsScene::createScene()));*/

    // 直接进入选择植物界面
    // uncomment after define SelectPlantsScene class
