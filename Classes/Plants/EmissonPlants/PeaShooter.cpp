...
PeaShooter::PeaShooter(Node* node) : _isCreatePea(false) {
    _node = node;
    _plantImage = nullptr;
    _healthPoint = 300;
...
void PeaShooter::plantAttack(Zombies* zombie) {
    if (getPlantIsSurvive() && zombie->getZombieIsSurvive() &&
        zombie->getZombieIsEnterMap() && /* 植物存活 && 僵尸没有死亡 && 僵尸进入地图 */
        getZombieIsTheFrontOfPlant(zombie) &&
        getZombieIsSameLineWithPlant(zombie)) /* 僵尸与植物在同一行 && 僵尸在植物的前方 */
    {
...
void PeaShooter::plantEmission(const string& plantAnimation) {
    ...
    _plantAnimation->setEventListener([&](spTrackEntry* entry, spEvent* event) {
        if (strcmp(event->data->name, "shoot") == 0) {
            rand() % 2 == 0 ? Bullet::playCollisionSoundEffect("throw") :
                              Bullet::playCollisionSoundEffect("throw2");
            createPea();
        }
    });
}
