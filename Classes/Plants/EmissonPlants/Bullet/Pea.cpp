...
Pea::Pea(Node* node) :
    _isFire(false), _torchwoodTag(0), _fireNumbers(0), _peaAnimationName("normal") {
    _node = node;
    _attack = 20;
...
void Pea::bulletInit() {
    ...
    _bulletAnimation = SkeletonAnimation::createWithData(animationData.find("FirePea")->second);
    ...
    _bulletAnimation->runAction(Sequence::create(
        MoveBy::create(3.0f, Vec2(2000, 0)),
        CallFunc::create([this]() { _bulletAnimation->setVisible(false); }), nullptr));
}

void Pea::createBulletShadow() {
    /* 创建影子 */
...
void Pea::bulletAndZombiesCollisionDetect() {
...
void Pea::bulletAttackZombies() {
    for (auto zombie : ZombiesGroup) {
        if (!_isUsed &&
            getBulletIsSameLineWithZombie(zombie) && /* 豌豆没有被使用 && 豌豆与僵尸在同一行 */
            getBulletIsEncounterWithZombie(zombie) && /* 豌豆与僵尸碰撞 */
            zombie->getZombieIsSurvive() &&
            zombie->getZombieIsEnterMap()) /* 僵尸没有死亡 && 僵尸进入地图内 */
        {
...
void Pea::fireBulletAttackZombies() {
    for (auto zombie : ZombiesGroup) {
        if (!_isUsed &&
            ...
            zombie->getZombieIsEnterMap()) /* 僵尸没有死亡 && 僵尸进入地图内 */
        {
            playCollisionSoundEffect(SoundEffectType::firepea); /* 播放指定音乐 */
...
void Pea::attackZombies() {
    for (auto zombie : ZombiesGroup) {
        if (getBulletIsSameLineWithZombie(zombie) && zombie->getZombieIsSurvive() &&
...
void Pea::createPeaExplode() {
    ...
    auto& animationData = _runtime->gameData->getAnimationData();
    auto peaExplode = SkeletonAnimation::createWithData(animationData.find("PeaExplode")->second);
    ...
    peaExplode->runAction(Sequence::create(
        DelayTime::create(0.8f),
        CallFunc::create([peaExplode]() { peaExplode->removeFromParent(); }), nullptr));
}

bool Pea::getIsFire() const { return _isFire; }

int Pea::getPeaFireNumbers() const { return _fireNumbers; }

int Pea::getTorchwoodTag() const { return _torchwoodTag; }

void Pea::setIsFire(const bool isFire) { _isFire = isFire; }

void Pea::setPeaFireNumbers(const int fireNumbers) { _fireNumbers = fireNumbers; }

void Pea::addPeaFireNumbers() { ++_fireNumbers; }

void Pea::setTorchwoodTag(const int torchwoodTag) { _torchwoodTag = torchwoodTag; }
