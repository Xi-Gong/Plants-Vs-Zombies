...
Zombies::Zombies() :
    _node(nullptr), _attackHeadSoundEffectType(0), _attackBodySoundEffectType(0),
    _bodyAnimationId(1), _bodyShieldAnimationId(1), _headShieldAnimationId(1), _isEat(false),
    _isEatGarlic(false), _isUseForGameType(false), _isShow(false), _isPreview(false),
    _isHaveShield(false), _isStrikeFly(false), _redWarning(false),
    _isShowLoseLimbsAnimation(true), _isCanDelete{false, false}, _zombieEatPlantNumber(-1),
    _zombieHowlNumbers(0), _highLightIntensity(0.3f), _highLightGLProgramState(nullptr),
    _highLightFinished(false), _headShieldType(ShieldType::none), _bodyShieldType(ShieldType::none),
    _levelDataHandler(LevelDataHandler::getInstance()),
    _runtime(Runtime::getInstance()), _animationName{"Zombies_Stand", "Zombies_Stand1",
                                                     "Zombies_Walk", "Zombies_Walk2"} {
    _random.seed(_device());
}

void Zombies::zombieInit(const string& animation_name) {
    std::uniform_real_distribution<float> number(0.f, 0.45f);
    _zombiesAnimation = SkeletonAnimation::createWithData(
        _runtime->gameData->getAnimationData().find(animation_name)->second);
...
void Zombies::setZombieScale(const int& scale) const { _zombiesAnimation->setScale(scale); }

void Zombies::setZombieTimeScale(const float& timeScale) const {
    _zombiesAnimation->setTimeScale(timeScale);
}

void Zombies::setZombieScale(const Size& scale) const { _zombiesAnimation->setContentSize(scale); }
...
void Zombies::setZombieMove(float delta) {
    ...
    if (!_redWarning && getZombieWarning()) {
        auto action = RepeatForever::create(Sequence::create(
            TintTo::create(0.3f, Color3B::RED), TintTo::create(0.3f, Color3B::WHITE), nullptr));
        action->setTag(10);
...
void Zombies::setZombiePosition(const Vec2& position) { _position = position; }

void Zombies::setZombieName(const string& name) { _zombieName = name; }

void Zombies::setZombieTag(const int& tag) { _zombieTag = tag; }

void Zombies::setZombieCurrentSpeed(const int currentSpeed) { _currentSpeed = currentSpeed; }

void Zombies::setZombieStop() { _currentSpeed = 0; }

void Zombies::setZombieIsShow(const bool isShow) { _isShow = isShow; }

void Zombies::setZombieIsEat(const bool isEat) { _isEat = isEat; }
...
void Zombies::setZombieVisible(const bool isVisible) { _zombiesAnimation->setVisible(isVisible); }

string Zombies::getZombieAniamtionName(ZombiesType zombiestype) { return "Zombies_Walk"; }
...
void Zombies::zombiesDeleteUpdate(list<Zombies*>::iterator& zombie) {
    ...
    if (!(*zombie)->getZombieAnimation()->isVisible()) {
        if (!(*zombie)->getIsCanDelete()[0]) {
            ...
            userWorldData.killZombiesNumbers += 1;
            ...
            if (++_zombiesNewDieNumbers >= 20 || getZombiesNumbers() <= 0) {
                ...
                UserDataHandler::getInstance()->caveUserData(
                    "KILLALLZOMBIES", userWorldData.killZombiesNumbers); /* 杀死僵尸数加一
                                                                          */
            }

            auto zombies = zombie;
            (*zombies)->getZombieAnimation()->runAction(Sequence::create(
                DelayTime::create(5.0f),
                CallFunc::create([zombies]() { (*zombies)->getIsCanDelete()[1] = true; }),
                nullptr));
        }
...
unsigned int Zombies::getZombiesNumbers() { return _zombiesNumbers; }

void Zombies::setZombiesNumbers(const unsigned int numbers) { _zombiesNumbers = numbers; }
...
bool Zombies::getZombieIsVisible() const { return _zombiesAnimation->isVisible() ? true : false; }

string Zombies::getZombieName() const { return _zombieName; }

int Zombies::getZombieTag() const { return _zombieTag; }

int Zombies::getZombieEatPlantNumber() const { return _zombieEatPlantNumber; }
...
void Zombies::setZombieIsStrikeFly(const bool isStrikeFly) { _isStrikeFly = isStrikeFly; }

void Zombies::setZombieHurtBlink() {
    if (!_highLightFinished) {
        _highLightFinished = true;
        auto action = Repeat::create(Sequence::create(CallFunc::create([this]() {
                                                          _highLightIntensity -= 0.06f;
                                                          _highLightGLProgramState->setUniformFloat(
                                                              "intensity", _highLightIntensity);
                                                      }),
        ...
        _zombiesAnimation->runAction(Sequence::create(
            ...
            DelayTime::create(0.15f), CallFunc::create([this]() {
...
void Zombies::setZombieScale() {
    _zombiesAnimation->setScale(_zombiesAnimation->getScale() +
                                (getZombieLocalZOrder(_position.y) + 10) / 20 / 40.f);
}

SkeletonAnimation* Zombies::getZombieAnimation() const { return _zombiesAnimation; }

Vec2 Zombies::getZombiePosition() const { return _zombiesAnimation->getPosition(); }

float Zombies::getZombiePositionX() const { return _zombiesAnimation->getPositionX(); }

float Zombies::getZombiePositionY() const { return _zombiesAnimation->getPositionY(); }

bool Zombies::getZombieIsEnterMap() const {
    return _zombiesAnimation->getPositionX() < 1710 ? true : false;
}

float Zombies::getZombieCurrentBodyShieldVolume() const { return _currentBodyShieldVolume; }

float Zombies::getZombieCurrentHeadShieldVolume() const { return _currentHeadShieldVolume; }

float Zombies::getZombieCurrentBloodVolume() const { return _currentBloodVolume; }

bool Zombies::getZombieIsEat() const { return _isEat; }

bool Zombies::getZombieIsStop() const { return _currentSpeed == 0 ? true : false; }

float Zombies::getZombieCurrentSpeed() const { return _currentSpeed; }

float Zombies::getZombieSpeed() const { return _speed; }

bool Zombies::getZombieIsShow() const { return _isShow; }

ZombiesType Zombies::getZombieType() const { return _zombiesType; }

int Zombies::getZombieHeadAttackSoundEffect() const { return _attackHeadSoundEffectType; }

int Zombies::getZombieBodyAttackSoundEffect() const { return _attackBodySoundEffectType; }

bool* Zombies::getIsCanDelete() { return _isCanDelete; }

bool Zombies::getZombieIsHaveShield() const { return _isHaveShield; }

bool Zombies::getZombieIsStrikeFly() const { return _isStrikeFly; }

void Zombies::playZombiesDieAnimation() {
    uniform_real_distribution<float> number(0.f, 0.4f);
    auto ashes = SkeletonAnimation::createWithData(
        _runtime->gameData->getAnimationData().find("Zombies_Ash")->second);
    ...
    ashes->setEventListener([ashes](spTrackEntry* entry, spEvent* event) {
        if (!strcmp(event->data->name, "Finished")) {
            ashes->runAction(Sequence::create(
                FadeOut::create(1.0f), CallFunc::create([ashes]() { ashes->setVisible(false); }),
                nullptr));
        }
    });

    ashes->runAction(Sequence::create(
        DelayTime::create(3), CallFunc::create([ashes]() { ashes->removeFromParent(); }), nullptr));
}
...
void Zombies::setZombieEatPlantNumber(const int& number) { _zombieEatPlantNumber = number; }
...
void Zombies::zombieLoseArmAnimation(const std::string& name) {
    ...
    {
        MusicPlayer::playMusic("limbs_pop");
        ...
        arm->runAction(Sequence::create(
            Spawn::create(
                Sequence::create(
                    JumpBy::create(0.5f, Vec2(-80 + rand() % 160, -100), rand() % 80 + 66, 1),
            ...
            CallFunc::create([arm]() { arm->removeFromParent(); }), nullptr));
        _node->addChild(arm);
...
void Zombies::zombieLoseHeadAnimation(const std::string& name) {
    if (_isShowLoseLimbsAnimation) /* 僵尸掉头 */
    {
        ...
        head->setScale(1.5f);
        head->runAction(Sequence::create(
            Spawn::create(
                Sequence::create(
                    JumpBy::create(0.5f, Vec2(-150 + rand() % 300, -120), rand() % 100 + 66, 1),
                    JumpBy::create(0.2f, Vec2(-20 + rand() % 40, 0), rand() % 10 + 10, 1), nullptr),
                RotateBy::create(0.5f, -180 + rand() % 360), nullptr),
            DelayTime::create(2.0f), FadeOut::create(0.5f),
            CallFunc::create([head]() { head->removeFromParent(); }), nullptr));
        _node->addChild(head);
...
void Zombies::zombieLoseShieldAnimation(const std::string& name) {
    ...
    cone->setScale(1.5f);
    cone->runAction(Sequence::create(
        Spawn::create(
            Sequence::create(
                JumpBy::create(0.5f, Vec2(-150 + rand() % 300, -170), rand() % 100 + 66, 1),
                JumpBy::create(0.2f, Vec2(-20 + rand() % 40, 0), rand() % 10 + 10, 1), nullptr),
            RotateBy::create(0.5f, -180 + rand() % 360), nullptr),
        DelayTime::create(2.0f), FadeOut::create(0.5f),
        CallFunc::create([cone]() { cone->removeFromParent(); }), nullptr));
    _node->addChild(cone);
...
void Zombies::zombieFadeOutAnimation() {
    ...
    _zombiesAnimation->setEventListener([&](spTrackEntry* entry, spEvent* event) {
        if (!strcmp(event->data->name, "filldown")) {
            ...
            MusicPlayer::playMusic(rand() % 2 ? "zombie_falling_1" : "zombie_falling_2");
        }
        if (!strcmp(event->data->name, "die")) {
            _zombiesAnimation->getChildByName("shadow")->runAction(FadeOut::create(1.0f));
            _zombiesAnimation->runAction(Sequence::create(
                FadeOut::create(1.0f),
                CallFunc::create([this]() { _zombiesAnimation->setVisible(false); }), nullptr));
        }
    });
}
...
ShieldType Zombies::getZombieBodyShieldType() const { return _bodyShieldType; }

ShieldType Zombies::getZombieHeadShieldType() const { return _headShieldType; }
...
void Zombies::showZombieShadow(Node* node, const int posy) {
    ...
    node->runAction(RepeatForever::create(Sequence::create(
        CallFunc::create([=]() { shadow->setPosition(Vec2(node->getPosition().x, positiony)); }),
        DelayTime::create(0.01f), nullptr)));

    shadow->runAction(
        Sequence::create(DelayTime::create(2.7f), FadeOut::create(0.5f), DelayTime::create(1.0f),
                         CallFunc::create([shadow]() { shadow->removeFromParent(); }), nullptr));
}
...
bool Zombies::getZombieWarning() { return _zombiesAnimation->getPositionX() < 570 + 122 * 2; }

GLProgram* Zombies::getHighlight() {
    auto program = GLProgramCache::getInstance()->getGLProgram("MyHighLightShader");
    if (!program) {
        program = GLProgram::createWithByteArrays(
