...
Car::Car(Node* node) :
    _carImage(nullptr), _position(Vec2::ZERO), _isLive(false), _tag(-1), _carState(1), _scale(0),
    _node(node), _runtime(Runtime::getInstance()) {}
...
void Car::setPosition(const Vec2& position) { _position = position; }

void Car::setCarTag(const int tag) { _tag = tag; }

void Car::setParent(Node* node) { _node = node; }

void Car::setScale(const float& scale) { _scale = scale; }

void Car::setLive(bool isLive) { _isLive = isLive; }

bool Car::getLive() const { return _isLive; }

int Car::getCarTag() const { return _carImage->getTag(); }

Sprite* Car::getCar() const { return _carImage; }

void Car::createCarListener() {
    for (auto& zombie : ZombiesGroup) {
        if (zombie->getZombieIsEnterMap() &&
            zombie->getZombieIsSurvive() && /* 僵尸进入地图 && 僵尸没有死亡 */
            getZombieIsSameLineWithCar(zombie) &&
            getzombieIsEncounterCar(zombie)) /* 僵尸与小车在同一行 && 僵尸遇到小车 */
        {
            _isLive = true;
...
void Car::carStartUp() {
    if (_isLive) {
        if (_carState == 1) {
            _carState = 2;
            MusicPlayer::playMusic("lawnmower");
...
bool Car::getZombieIsSameLineWithCar(Zombies* zombie) const {
    return fabs((zombie->getZombieAnimation()->getPositionY() + 50) - _carImage->getPositionY()) <=
                   10 ?
               true :
               false;
}
...
bool Car::getzombieIsEncounterCar(Zombies* zombie) const {
    return zombie->getZombieAnimation()->getPositionX() - 80 < _carImage->getPositionX() ? true :
                                                                                           false;
}
